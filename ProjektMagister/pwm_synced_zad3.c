...
void setupGPIO(){
    ...
    //EPWM6A
    GpioCtrlRegs.GPAMUX1.bit.GPIO10=0x01; // PWM6A SET
    GpioCtrlRegs.GPAPUD.bit.GPIO10=1; //pull up reseistor off
    GpioCtrlRegs.GPADIR.bit.GPIO10=1; // pin output direction

    //EPWM6B
    GpioCtrlRegs.GPAMUX1.bit.GPIO11=0x01; //epwm6B
...
void setupPWM1(){
    ...
    EPwm1Regs.CMPA.half.CMPA=100;         // 1500
...
void setupPWM2(){
    ...
    EPwm2Regs.TBPRD=7500;            // 7500
    EPwm2Regs.CMPA.half.CMPA=100;         // 1500
    ...
    EPwm2Regs.TBPHS.half.TBPHS =  5002; // 7500/3 +2 = 2502
    EPwm2Regs.TBCTL.bit.PHSDIR = 1; //1 for pwm2 before pwm1, 0 for pwm2 after pwm1
...
void setupPWM6(){
    EALLOW;
    //PWM CONFIG
    EPwm6Regs.TBCTL.bit.CTRMODE=2;    // set up-down mode
    EPwm6Regs.TBCTL.bit.HSPCLKDIV=0x00;  // 10
    EPwm6Regs.TBCTL.bit.CLKDIV=0x00;     // 32
    EPwm6Regs.TBCTL.bit.FREE_SOFT=0x02;  // do not stop when cpu is halted
    EPwm6Regs.AQCTLA.bit.CAU=0x02;       // disable output A when compare A and counting UP
    EPwm6Regs.AQCTLA.bit.CAD=0x01;       // enable output A when compare A and counting DOWN
    EPwm6Regs.TBPRD=7500;            // 7500
    EPwm6Regs.CMPA.half.CMPA=100;         // 1500

    //DEAD - BAND setup
    EPwm6Regs.DBCTL.bit.IN_MODE=0x00; //EPWMA source for both rising and falling
    EPwm6Regs.DBCTL.bit.POLSEL=0x02;  //AHC mode, epwmb is inverted
    EPwm6Regs.DBCTL.bit.OUT_MODE=0x03; //deadband fully on
    EPwm6Regs.DBFED=750; //750 cycles TBCLK 5us
    EPwm6Regs.DBRED=750; //750 cycles TBCLK 5us

    //ADC
    EPwm6Regs.ETSEL.bit.SOCAEN = 1; // soca enable
    EPwm6Regs.ETSEL.bit.SOCASEL = 0x02; //adc start when TBPRD
    EPwm6Regs.ETPS.bit.SOCACNT = 0x01; // 1 event has occured
    EPwm6Regs.ETPS.bit.SOCAPRD = 0x01; // 1 event has occured
    EPwm6Regs.ETPS.bit.INTCNT = 0x01; // 1 event has occured
    EPwm6Regs.ETPS.bit.INTPRD = 0x01; // 1 event has occured

    //TRIPZONE
    EPwm6Regs.TZSEL.bit.OSHT1 = 1; // tripzone1 ONE SHOT enable
    EPwm6Regs.TZCTL.bit.TZA = 0x02; //pwmA tripzone to low state
    EPwm6Regs.TZCTL.bit.TZB = 0x02; // pwmB tripzone to low state
    EPwm6Regs.TZEINT.bit.OST = 1; //enable cyclebycycle interrupts
    ...
    //SYNC PWM1(MASTER) WITH PWM2(SLAVE)
    EPwm6Regs.TBCTL.bit.PHSEN = 1; //turn on sync
    EPwm6Regs.TBPHS.half.TBPHS =  5002; // 7500/2 +2 = 3752
    EPwm6Regs.TBCTL.bit.PHSDIR = 0; //1 for pwm2 before pwm1, 0 for pwm2 after pwm1
...
__interrupt void adc_isr(void)
{
    ...
    EPwm1Regs.CMPA.half.CMPA = ADCresult;
