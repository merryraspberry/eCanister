...
void setupGpioUart(){
    ...
    //EPWM5B
    GpioCtrlRegs.GPAMUX1.bit.GPIO9=0x01; // PWM5B SET
    GpioCtrlRegs.GPAPUD.bit.GPIO9=1; //pull up reseistor off
    GpioCtrlRegs.GPADIR.bit.GPIO9=1; // pin output direction
...
void setupPWM5(){
    EALLOW;
    //PWM CONFIG
    EPwm5Regs.TBCTL.bit.CTRMODE=2;    // set up-down mode
    EPwm5Regs.TBCTL.bit.HSPCLKDIV=0x00;  // 1
    EPwm5Regs.TBCTL.bit.CLKDIV=0x00;     // 1 75mhz
    EPwm5Regs.TBCTL.bit.FREE_SOFT=0x02;  // do not stop when cpu is halted
    EPwm5Regs.AQCTLB.bit.CAU=0x02;       // disable output A when compare A and counting UP
    EPwm5Regs.AQCTLB.bit.CAD=0x01;       // enable output A when compare A and counting DOWN
    EPwm5Regs.TBPRD = 7500;            // 7500
    EPwm5Regs.CMPA.half.CMPA=0;         // 0% DUTY for start

    //ADC
    EPwm5Regs.ETSEL.bit.SOCAEN = 1; // soca enable
    EPwm5Regs.ETSEL.bit.SOCASEL = 0x02; //adc start when TBPRD
    EPwm5Regs.ETPS.bit.SOCACNT = 0x01; // 1 event has occured
    EPwm5Regs.ETPS.bit.SOCAPRD = 0x01; // 1 event has occured
    EPwm5Regs.ETPS.bit.INTCNT = 0x01; // 1 event has occured
    EPwm5Regs.ETPS.bit.INTPRD = 0x01; // 1 event has occured
...
__interrupt void SCI_ISR(){
    ...
    EALLOW;
    EPwm5Regs.CMPA.half.CMPA=RX_char*30;
    EDIS;
}
