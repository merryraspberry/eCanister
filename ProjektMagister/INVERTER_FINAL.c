//THIS PROGRAM WORKS FOR BOTH BIPOLAR AND UNOPOLAR PLECS MODELS, IT DEPENDS ON PLECS WHETHER IT WILL USE PWM1 ONLY AND CREATE NEGATION
//OUT OF IT OR TAKE PWM1 AND PWM2 CREATED BY MICROCONTROLLER
...
float reference_sinus = 0;
float Vmod = 0;
float Vmod0_to_1 = 0;
float Vmod0_to_1_inverted = 0;
...
__interrupt void adc_isr(void)
{
    ...
    reference_sinus = sin(angle); // sin wave -1 to 1
    Vsinout = reference_sinus*Vsin; //from -350V to 350V
    Vmod = Vsinout/ADCresultVin3;
    Vmod0_to_1 = (Vmod+1)/2;
    Vmod0_to_1_inverted = 1 - Vmod0_to_1;
    pulse_width = Vmod0_to_1*3750; //according to max pwm counter 3750 set now
    pulse_width_inverted = Vmod0_to_1_inverted*3750; //according to max pwm counter 3750 set now


    //reference_sinus_0_to_1 = (sin(angle)+1)/2; //sin wave 0 to 1
    //reference_sinus_0_to_1_inverted = 1 - reference_sinus_0_to_1; //reversed sin wave 0 to 1
